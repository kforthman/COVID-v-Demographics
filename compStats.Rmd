...
compileStats <- function(state, stat){
  day <- substring(daily_filenames,1,10)
  dailystat <- data.frame(day = day, stat = NA)
  for(i in 1:length(daily_filenames)){
    daysReport <- read.csv(
      paste0(data.path, daily_filenames[i]),
      header = T, stringsAsFactors = F)
    dailystat[i,"stat"] <- daysReport[daysReport$Province_State == state, stat]
  }
  return(dailystat)
}
```
...
plot.dailyStat <- function(state, stat){
  data <- compileStats(state, stat)
  barplot(data$stat, names.arg = data$day, main = paste0(state, " ", stat), las = 2, cex.axis = 1, cex.names = 0.5)
}

plot.dailyStatRise <- function(state, stat){
  data <- compileStats(state, stat)
  
  rise.stat <- matrix(ncol = nrow(data) - 1, nrow = 1)
  colnames(rise.stat) <- data$day[-1]
  for(i in 1:ncol(rise.stat) + 1){
    rise <- data$stat[i] - data$stat[i-1]
    rise.stat[i-1] <- rise
  }
...
```{r}
plot.dailyStat("Oklahoma", "People_Tested")
plot.dailyStatRise("Oklahoma", "People_Tested")

plot.dailyStat("Oklahoma", "Testing_Rate")

plot.dailyStat("New York", "People_Tested")
plot.dailyStatRise("New York", "People_Tested")

plot.dailyStat("New York", "Testing_Rate")
```
...
```{r}
plot.dailyStat("Oklahoma", "People_Hospitalized")
plot.dailyStatRise("Oklahoma", "People_Hospitalized")

plot.dailyStat("Oklahoma", "Hospitalization_Rate")

plot.dailyStat("New York", "People_Hospitalized")
plot.dailyStatRise("New York", "People_Hospitalized")

plot.dailyStat("New York", "Hospitalization_Rate")
```
